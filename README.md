...
### Three witnesses to a visit

`server_logs` holds three witnesses, each writing its own sessions under its own `source`:

| Source    | Writer                                                                                                     | Sees                                                                                      |
| --------- | ---------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| `server`  | CR Roleplay webhook                                                                                        | Exact connect and exit, when it fires                                                     |
| `cfx`     | Bot, from consistent Cfx.re rosters: open after 2 consecutive listings, close after 3 consecutive absences | Every player on the server, late by the directory cache; dated to first and last sighting |
| `discord` | Bot, from Discord activity: close after 3 consecutive reads without it                                     | Only members sharing activity and not invisible                                           |

Trust runs `server`, then `cfx`, then `discord`, and every reader applies it:

- **Playtime** reads the `server_visits` view (migration 000040). A `cfx` or `discord` visit edge within 10 minutes of a webhook edge for the same character is snapped to the webhook's time, so the weaker witnesses' lag is never credited; where the webhook said nothing they count on their own. The recap, `!check`, MY RECORDS and the dashboard all union these visits per character.
- **Dashboard status** comes from the most trusted live visit. A weaker visit that opened before the webhook's latest exit is that visit's lagging tail and cannot hold the player online; one that opened after it is a visit the webhook missed.
- **The server log channel** posts each change once. The first witness to report a connect, connected or exit posts it; another witness reporting the same change for the same character within 10 minutes edits that message, the footer naming every witness (`Source: CR Roleplay + Discord activity`) and the most trusted one's slot, reason and playtime shown. A weaker witness's exit while a stronger one still has the player on the server - activity switched off, a member gone invisible - is not announced, nor is its connect more than 10 minutes into a visit the stronger one already reported. The posted messages are held in memory, so a bot restart costs at most one duplicate per change in flight.

A Discord or CFX sighting is credited only to a character the webhook has reported at least once: the webhook is still what creates `server_members` rows. Discord activity goes to the character the webhook or CFX currently has on the server, else the most recently seen one, and a Discord visit always closes on the character it opened on. Webhook events only ever join the webhook's own sessions.
...
`/stash` is the only way to read or move safebox stock, and it works only inside the two safebox channels. There is deliberately no `!stash` prefix form: a prefix command would take the item as text, and a member writing their own word for an item can name something the database does not hold. Every item in the flow is picked from a menu the bot fills out of `safebox_stock_items`, so an item is a stored row by construction.

The channel fixes the safebox - `STASH_PUBLIC_CHANNEL_ID` is the public stash and nothing else - so the command never names it. The action comes from the subcommand alone and is held on the draft, which is why a builder whose draft has expired refuses to submit and asks for `/stash deposit` or `/stash withdraw` again rather than guessing.
...
Both channels are a stock ledger, not a chat. Any message written in one - conversation, an image, or another bot command such as `!recap` - is answered with a reply naming the safebox and the two commands that record something, and nothing else runs there. The member's own message is left alone; the bot deletes its own notice after 15 seconds, so the channel needs no moderation permission and a false positive destroys nothing. Nobody is exempt, admins included, and every offending message is answered.
